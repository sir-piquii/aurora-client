...
import { getProductById, addQuotation, BASE_URL } from "../api";

/**
 * ProductDetailView component displays detailed information about a single product,
 * including images, description, benefits, warranty, and pricing (for dealers).
 *
 * Features:
 * - Fetches product data by ID from the backend.
 * - Displays product images with selectable thumbnails.
 * - Shows product details such as name, description, benefits, warranty, and datasheet download link.
 * - Allows users to add the product to a cart (stored in localStorage).
 * - Provides a modal form for requesting a quote, pre-filling user information if available.
 * - Handles quote submission and displays success/error alerts.
 * - Adjusts displayed price based on user role (dealer).
 *
 * State:
 * - product: The product object fetched from the backend.
 * - loading: Boolean indicating if the product data is being loaded.
 * - selectedImage: The currently selected product image.
 * - userRole: The role of the logged-in user (e.g., "dealer").
 * - showModal: Boolean indicating if the quote request modal is open.
 * - quoteForm: Object containing the quote request form fields.
 *
 * Side Effects:
 * - Sets the document title on mount.
 * - Fetches product data when the productId changes.
 * - Sets the initial selected image when product data is loaded.
 *
 * @component
 * @returns {JSX.Element} The rendered ProductDetailView component.
 */
        ...
        {loading ? (
          ...
          <div className="flex flex-col md:flex-row">
            ...
            <div className="md:w-1/2">
              <img
                src={`${BASE_URL}/products/${selectedImage}`}
