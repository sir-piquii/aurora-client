/**
 * CaseStudies component for managing and displaying a paginated list of case studies in the admin panel.
 *
 * Features:
 * - Fetches case studies from the API on mount.
 * - Displays case studies in a paginated table.
 * - Allows deletion of case studies with confirmation.
 * - Provides navigation to add and edit case studies.
 *
 * State:
 * @typedef {Object} CaseStudy
 * @property {number|string} id - Unique identifier for the case study.
 * @property {string} projectName - Name of the project.
 * @property {string} location - Location of the project.
 * @property {string|number} systemCapacity - System capacity of the project.
 *
 * @component
 *
 * @returns {JSX.Element} The rendered CaseStudies admin management table.
 */
    ...
    if (confirmDelete) {
      const deleteCaseStudyAsync = async () => {
        try {
          ...
          setCaseStudies(
            caseStudies.filter((caseStudy) => caseStudy.id !== caseStudyId)
          );
  ...
  return (
    <div className="max-w-7xl mx-auto my-12 px-4">
            ...
            {currentCaseStudies.length > 0 ? (
              currentCaseStudies.map((caseStudy) => (
                <tr key={caseStudy.id} className="hover:bg-gray-50">
                  <td className="p-3 border">{caseStudy.id}</td>
                  <td className="p-3 border">{caseStudy.projectName}</td>
                  <td className="p-3 border">{caseStudy.location}</td>
                  <td className="p-3 border">{caseStudy.systemCapacity}</td>
      ...
      <div className="flex justify-center mt-6 space-x-2">
        {Array.from(
          {
            length: Math.ceil(caseStudies.length / caseStudiesPerPage),
          },
          (_, i) => (
            <button
              key={i}
