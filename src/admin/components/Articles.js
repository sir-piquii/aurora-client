/**
 * Articles component for the admin panel.
 *
 * Displays a paginated list of articles with options to add, edit, and delete articles.
 * Fetches articles from the API on mount and updates the document title.
 *
 * Features:
 * - Fetches articles from the backend API.
 * - Displays articles in a table with pagination.
 * - Allows deletion of articles with confirmation.
 * - Provides navigation to add and edit article pages.
 *
 * @component
 * @returns {JSX.Element} The rendered Articles management component.
 */
...
const Articles = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto my-12 px-4">
      <h2 className="text-2xl font-bold mb-6 text-navy-900">Manage Articles</h2>
      <Link
            ...
            {currentArticles.length > 0 ? (
              currentArticles.map((article) => (
                <tr key={article.article_id} className="hover:bg-gray-50">
                  <td className="p-3 border">{article.article_id}</td>
                  <td className="p-3 border">{article.article_title}</td>
                  <td className="p-3 border">{article.category}</td>
